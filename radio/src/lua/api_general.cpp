...
@param path (string) full path to wav file (i.e. â€?SOUNDS/en/system/tada.wavâ€?
...
@param input (number) value that can be adjusted by the +/Â­- keys

@param min  (number) min value that input can reach (by pressing the -Â­ key)
...
This function reads/writes the Multi protocol buffer to interact with a protocolÂ².
